 ...
 *
 * After wallpaper JSON is applied, the Donwaztok top-level palette (donwaztok/config.json) overwrites
 * Appearance.m3colors so qs.modules.* widgets match the M3 bar/drawers.
 */
Singleton {
    id: root
    property string filePath: Directories.generatedMaterialThemePath

    // Omit *_paletteKeyColor / neutral_* — donwaztok Colours has them; shared Appearance.m3colors does not.
    ...
    function applyColors(fileContent) {
        ...
        Qt.callLater(() => root.applyDonwaztokPaletteToSharedUi())
    }
