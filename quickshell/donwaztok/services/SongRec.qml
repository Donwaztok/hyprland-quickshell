...
Singleton {
    ...
    function handleRecognition(jsonText) {
        try {
            ...
            Quickshell.execDetached(["notify-send", qsTr("Couldn't recognize music"), qsTr("Perhaps what you're listening to is too niche"), "-a", "Shell"])
        }
    }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 1) {
                Quickshell.execDetached(["notify-send", qsTr("Couldn't recognize music"), qsTr("Make sure you have songrec installed"), "-a", "Shell"])
            }
        }
    }
