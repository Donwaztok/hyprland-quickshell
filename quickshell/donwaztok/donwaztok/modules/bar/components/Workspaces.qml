...
Item {
    ...
    property bool borderless: DwCfg.Config.bar.borderless
    ...
    readonly property string workspaceStyle: DwCfg.Config.bar.workspaces.style || "gnome"
    readonly property bool isGnomeStyle: root.workspaceStyle === "gnome"

    readonly property int workspacesShown: Math.max(1, DwCfg.Config.bar.workspaces.shown)
    readonly property bool isDynamicMode: root.isGnomeStyle && (DwCfg.Config.bar.workspaces.shown === 0)
    ...
    property int workspaceButtonWidth: (DwCfg.Config.bar.workspaces.workspaceButtonWidth ?? 11)
    property int activeSlotWidth: (DwCfg.Config.bar.workspaces.activeSlotWidth ?? 24)
    property real activeWorkspaceMargin: 1
    property real dashWidthFactor: (DwCfg.Config.bar.workspaces.dashWidthFactor ?? 2.0)
    property real dashMargin: (DwCfg.Config.bar.workspaces.dashMargin ?? 1)
    readonly property real indicatorSize: (DwCfg.Config.bar.workspaces.indicatorSize ?? DwCfg.Config.bar.workspaces.dotSize ?? DwCfg.Config.bar.workspaces.pillHeight ?? 6)
    ...
    property int classicSlotWidth: DwCfg.Config.bar.workspaces.classicSlotWidth ?? 26
    ...
    readonly property int barLane: DwCfg.Config.bar.sizes.thickness
    ...
    Timer {
        id: showNumbersTimer
        interval: DwCfg.Config.bar.workspaces.superKey.delayMs ?? 140
        repeat: false
    ...
    Connections {
        ...
        function onSuperDownChanged() {
            if (!DwCfg.Config.bar.workspaces.superKey.showNumbers) return
            ...
            Repeater {
                ...
                Button {
                    ...
                    background: Item {
                        ...
                        StyledText {
                            opacity: (root.showNumbers || DwCfg.Config.bar.workspaces.alwaysShowNumbers || (DwCfg.Config.bar.workspaces.showAppIcons && classicCell.biggestWindow && root.showNumbers) || (root.showNumbers && !DwCfg.Config.bar.workspaces.showAppIcons)) ? 1 : 0
                            anchors.centerIn: parent
                            horizontalAlignment: Text.AlignHCenter
                            verticalAlignment: Text.AlignVCenter
                            font.pixelSize: Appearance.font.barPixelSize.small - ((text.length - 1) * (text !== "10") * 2)
                            font.family: DwCfg.Config.bar.workspaces.useNerdFont ? Appearance.font.family.iconNerd : "sans-serif"
                            text: (DwCfg.Config.bar.workspaces.numberMap || [])[classicCell.wsValue - 1] || classicCell.wsValue
                        ...
                        Rectangle {
                            ...
                            radius: width / 2
                            color: root.effectiveActiveWorkspaceId === classicCell.wsValue ? Appearance.m3colors.m3onPrimary : (root.workspaceOccupied[classicCell.wsIndex] ? Appearance.m3colors.m3onSecondaryContainer : Appearance.colors.colOnLayer1Inactive)
                            opacity: (DwCfg.Config.bar.workspaces.alwaysShowNumbers || root.showNumbers || (DwCfg.Config.bar.workspaces.showAppIcons && classicCell.biggestWindow)) ? 0 : 1
                        }
                        IconImage {
                            visible: DwCfg.Config.bar.workspaces.showAppIcons && classicCell.biggestWindow
