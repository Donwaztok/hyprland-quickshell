...
QuickToggleModel {
    ...
    name: qsTr("Identify Music")
    statusText: toggled ? qsTr("Listening...") : sourceIsMonitor ? qsTr("System sound") : qsTr("Microphone")
    icon: toggled ? "music_cast" : (sourceIsMonitor ? "music_note" : "frame_person_mic")

    tooltipText: qsTr("Recognize music | Right-click to toggle source")
