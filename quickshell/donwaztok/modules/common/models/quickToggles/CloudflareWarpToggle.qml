...
QuickToggleModel {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                Quickshell.execDetached(["notify-send", 
                    qsTr("Cloudflare WARP"), 
                    qsTr("Connection failed. Please inspect manually with the <tt>warp-cli</tt> command")
                    , "-a", "Shell"
                ])
            }
        }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            if (exitCode === 0) {
                ...
                Quickshell.execDetached(["notify-send", 
                    qsTr("Cloudflare WARP"), 
                    qsTr("Registration failed. Please inspect manually with the <tt>warp-cli</tt> command"),
                    "-a", "Shell"
                ])
            }
        }
    ...
    tooltipText: qsTr("Cloudflare WARP (1.1.1.1)")
}
