...
Pane {
    ...
    Item {
        ...
        Image {
            ...
            source: (function () {
                var raw = config.BackgroundS
                if (raw !== undefined && raw !== null && String(raw).trim() !== "") {
                    var paths = String(raw).trim().split(/\s+/).filter(function (p) {
                        return p.length > 0
                    })
                    if (paths.length > 0)
...
// Based on SDDM Eucalyptus Drop (Qt6): https://gitlab.com/Matt.Jolly/sddm-eucalyptus-drop/
// Sugar Candy palette, wallpapers, and layout defaults from SDDM Sugar Candy (Marian Arlt).
//
