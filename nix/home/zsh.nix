...
}:
{
  programs.zsh = {
    ...
    shellAliases = {
      ...
      up = "sudo nixos-rebuild switch --flake path:${config.home.homeDirectory}/.config#${local.flakeHost}";
      up-test = "sudo nixos-rebuild test --flake path:${config.home.homeDirectory}/.config#${local.flakeHost}";
      pl = "nix profile list";
